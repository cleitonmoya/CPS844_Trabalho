...
Algoritmo Pocket PLA (com inicialização por regressão linear)
"""
import numpy as np
...
nit =  50       # número de iterações máximas de cada experimento
...
pr = 0.1        # probabilidade de ruído nos dados de treinamento
...
for n in range(nexp):
    ...
    # 5. Executa o Pocket PLA
    # ---------------- Pocket Perceptron Learning Algorithm ----------------- 
    # 5.0 Hipótese inicial
    if Reg:
        w = regressaoLinear(X,Y) # inicializa 'w' por regressão linear 
    else:
        w = np.array([0,0,0])
    wb = w    # armazena o melhor w em termos de Ein
    h0 = h(w) # hipótese inicial
    H = h0(X) # vetor com as hipóteses iniciais caculadas
    Ein_b = np.mean(H!=Y) # Ein inicial
    
    for t in range(nit):        
        
        # 5.1 escolhe um ponto mal-classificado, se existir
        ...
        # 5.2 atualiza o vetor de pesos
        w = w + y*x
       
        # 5.3 atualiza a função-hipótese
        hm = h(w) # hipótese m
        H = hm(X) # classificação de 'X'
        
        # 5.4 Avalia a hipótese com base no Ein b
        Ein_t = np.mean(H!=Y)
        if Ein_t < Ein_b: # caso w melhor que wb, 'coloca no bolso'
            wb = w
            Ein_b = Ein_t        
        
    # 5.5 Seleciona a hipótese final = melhor hipótese
    g = h(wb)      
    g_ = h_(wb)
    #  -------------------------- fim do PLA Pocket -------------------------- 
    
    # 6. Armazena o Ein do experimento:
    Ein = np.append(Ein,Ein_t)
    
    # 7. Avalia Eout do experimento:
    if Nout > 0:
