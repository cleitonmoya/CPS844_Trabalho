...
def h(w):
    # Suando x é um vetor, a fómula é h = sign(w.T@x), e h é um escalar.
    # Se usarmos X como matriz, necessitamos usar a transposta de X para
    # o cálculo do vetor h. 
    h = lambda x: np.sign((w.T@x.T).T)
    return h
...
def fi2(x1,x2): 
    if np.size(x1)==1:
        fi_x = np.array([[1], [x1], [x2], [x1*x2], [x1**2], [x2**2]])
    else:
        # para o caso de X1 ou X2 na forma vetorial
        fi_x = np.array([np.ones(np.size(x1)).T, x1.T, x2.T, (x1*x2).T, (x1**2).T, (x2**2).T])
    return fi_x
...
for n in range(nexp):
    ...
    # 4. Realiza a transformação não-linear fi(x)
    Z = fi(X1,X2)
    
    # 5. Calcula a hipótese w~(z) por gressão linear
    w_til_n = regressaoLinear(Z, Y)
    w_til = np.append(w_til,w_til_n) #adiciona w_til_n no vetor w_til
    ...
    g = lambda x1,x2: np.sign(w_til_n@fi(x1,x2))
    
    # 8. Avalia Ein do experimento:
    Ein_n = np.mean(G_til!=Y)
    Ein = np.append(Ein, Ein_n)
    
    # 9. Avalia Eout do experimento:
    Xout, Yout, Eout_n = avaliaEoutExp(f,g,Nout,pr)
    Eout = np.append(Eout, Eout_n)    

# Calcula o w_til médio
w_til2 = w_til.reshape(nexp,6)
w_til_mean = np.array([])
for j in range(6):
    w_til_mean = np.append(w_til_mean, w_til2[:,j].mean())
